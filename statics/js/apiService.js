...
const ApiService = (() => {
  ...
  async function _request(url, opts = {}) {
    try {
      ...
      // Convert browser NetworkError into a readable message
      if (err instanceof TypeError && err.message && (
        ...
        throw new Error(
          `Cannot reach server at ${window.location.origin}. ` +
          `Make sure ThreatWeave is running and you are on the same network. ` +
          `(${err.message})`
        );
      }
      throw err;
  ...
  async function startScan(target, scanType, projectName = '') {
    // Scans can take 30-90 minutes — use no timeout on the fetch itself.
    // The server handles nmap timeouts internally and always returns a response.
    return _request(`${BASE}/scan`, {
      method: 'POST',
      body: JSON.stringify({ target, scan_type: scanType, project_name: projectName }),
      // keepalive ensures the request survives page navigation/tab switch
      keepalive: false,
    });
  }
