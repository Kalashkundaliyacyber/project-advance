...
const Graphs = (() => {
  ...
  function buildOsintTree(riskData) {
    ...
    // Fix 8: Show "Building graph…" overlay immediately before the
    // synchronous D3 tree computation blocks the main thread.
    // setTimeout(..., 0) yields to the browser so the overlay paints
    // before the heavy work starts (important for 50+ port / 200+ CVE scans).
    ...
    setTimeout(() => {
      try {
        _buildOsintTreeSync(riskData);
      } finally {
        // Always hide overlay — even if build threw
        if (loadingEl) loadingEl.style.display = 'none';
      }
    }, 0);
  }

  /** Internal synchronous build — called inside the deferred setTimeout. */
  function _buildOsintTreeSync(riskData) {
  ...
  /* ══════════════════════════════════════════════════════════════
     SECTION D — SCOPED OSINT GRAPH  (for inline /graph chat card)
     Completely isolated from Section C (_osint global state).
     Each call creates its own controller + closure state so that
     multiple graph cards can coexist in the chat without ID clashes.
     Uses class-based querySelector instead of getElementById.
  ══════════════════════════════════════════════════════════════ */

  /**
   * Public entry-point called by _cmdGraph in chatbot.js.
   * containerEl — the chat card <div> that contains .gc-canvas,
   *               .gc-svg, .gc-zoom-g, .gc-links, .gc-nodes,
   *               .gc-loading, .gc-empty, .gc-status.
   * Attaches the controller to containerEl._osintCtrl so the
   * toolbar buttons (expand/collapse/reset) can reach it.
   */
  function buildOsintTreeInContainer(riskData, containerEl) {
    if (!riskData?.hosts?.length) return;
    const loadEl  = containerEl.querySelector('.gc-loading');
    const emptyEl = containerEl.querySelector('.gc-empty');
    if (emptyEl)  emptyEl.style.display = 'none';
    if (loadEl)   loadEl.style.display  = 'flex';
    // Yield to browser so loading spinner paints before heavy D3 work
    setTimeout(() => {
      try {
        const ctrl = _makeScopedCtrl(containerEl);
        ctrl.build(riskData);
        containerEl._osintCtrl = ctrl;
      } finally {
  ...
  /**
   * Factory — creates an isolated OSINT controller for one container.
   * All D3 selections and state are private to this closure.
   */
  function _makeScopedCtrl(containerEl) {
    let _root = null;
    let _svg = null, _zoomG = null, _zoom = null;
    let _idCtr = 0, _fitted = false;
    const q = sel => containerEl.querySelector(sel);

    /* ── Build: construct tree data + initialise D3 ─────────── */
    function build(riskData) {
      _root  = _buildOsintDataWith(riskData, () => ++_idCtr);
      _svg   = d3.select(q('.gc-svg'));
      _zoomG = d3.select(q('.gc-zoom-g'));
      d3.select(q('.gc-links')).selectAll('*').remove();
      d3.select(q('.gc-nodes')).selectAll('*').remove();
      _zoom = d3.zoom().scaleExtent([0.08, 5])
        .on('zoom', e => _zoomG.attr('transform', e.transform));
      _svg.call(_zoom).on('dblclick.zoom', null);
      _render();
      const host  = riskData.hosts[0] || {};
      const ports = host.ports || [];
      const cves  = ports.reduce((a, p) => a + (p.cves || []).length, 0);
      const st = q('.gc-status');
      if (st) st.textContent =
        `${host.ip || '?'} · ${ports.length} ports · ${cves} CVEs · Click any node to explore`;
    }

    /* ── Render: D3 layout + enter/update/exit ───────────────── */
    function _render() {
      if (!_root) return;
      const canvas = q('.gc-canvas');
      const W = canvas.clientWidth  || 820;
      const H = canvas.clientHeight || 460;

      const visibleCh = n =>
        (!n._collapsed && n.children?.length) ? n.children : null;

      const hier   = d3.hierarchy(_root, visibleCh);
      const leaves = hier.leaves().length;
      const treeH  = Math.max(H - 60, leaves * 22);
      const treeW  = Math.max(W * 0.78, 600);
      d3.tree().size([treeH, treeW])(hier);

      const linkGen = d3.linkHorizontal().x(d => d.y).y(d => d.x);
      const linksG  = d3.select(q('.gc-links'));
      const nodesG  = d3.select(q('.gc-nodes'));

      // Links
      const links = linksG.selectAll('path')
        .data(hier.links(), d => `${d.source.data.id}-${d.target.data.id}`);
      links.enter().append('path').attr('d', linkGen)
        .attr('fill', 'none').attr('stroke', '#2d3748').attr('stroke-width', 0.8)
        .style('opacity', 0).transition().duration(280).style('opacity', 1);
      links.transition().duration(280).attr('d', linkGen);
      links.exit().transition().duration(200).style('opacity', 0).remove();

      // Nodes
      const filterId = q('.gc-svg')?.dataset?.filterId || 'osint-glow';
      const nodes = nodesG.selectAll('g.onode')
        .data(hier.descendants(), d => d.data.id);

      const enter = nodes.enter().append('g').attr('class', 'onode')
        .attr('transform', d => `translate(${d.y},${d.x})`)
        .style('opacity', 0)
        .style('cursor', d => d.data._hasChildren ? 'pointer' : 'default')
        .on('click',      (e, d) => { e.stopPropagation(); _toggle(d.data.id); })
        .on('mouseenter', (e, d) => _osintTip(e, d, true))
        .on('mouseleave', ()     => _osintTip(null, null, false));

      // Host outer glow ring
      enter.filter(d => d.data.type === 'host').append('circle')
        .attr('r', d => _oRadius(d.data.type) + 9)
        .attr('fill', 'none').attr('stroke', d => _oColor(d) + '44').attr('stroke-width', 2);

      // Label (left of dot; host labels right as it is the root)
      enter.append('text').attr('class', 'osint-label')
        .attr('x', d => {
          const r = _oRadius(d.data.type);
          return d.data.type === 'host' ? r + 10 : -(r + 8);
        })
        .attr('dy', '0.35em')
        .attr('text-anchor', d => d.data.type === 'host' ? 'start' : 'end')
        .attr('font-size', d => ({
        ...
        .attr('fill', '#e6edf3')
        .attr('font-weight', d =>
          ['host', 'port', 'cve', 'attack'].includes(d.data.type) ? '600' : '400')
        .text(d => {
          const max = {
            host: 26, port: 32, version: 28, cve: 20,
            detail: 40, detail_fix: 40, patch: 40, attack: 26,
      ...
      // Main dot
      enter.append('circle').attr('class', 'osint-dot')
        .attr('r', d => _oRadius(d.data.type))
        .attr('fill', d => _oColor(d))
        .attr('stroke', d => _oColor(d) + 'aa')
        .attr('stroke-width', d => d.data.type === 'host' ? 2.5 : 1.5)
        .attr('filter', d => d.data.type === 'host' ? `url(#${filterId})` : null);

      // +/− expand indicator badge
      enter.filter(d => d.data._hasChildren).append('text').attr('class', 'expand-sign')
        .attr('x', 0).attr('y', '0.32em').attr('text-anchor', 'middle')
        .attr('font-size', d => _oRadius(d.data.type) > 7 ? '8px' : '6px')
        .attr('fill', '#0f1623').attr('font-weight', '800').attr('pointer-events', 'none')
        .text(d => d.data._collapsed ? '+' : '−');

      enter.transition().duration(280)
        .attr('transform', d => `translate(${d.y},${d.x})`).style('opacity', 1);

      nodes.transition().duration(280)
        .attr('transform', d => `translate(${d.y},${d.x})`);
      nodes.select('.expand-sign').text(d => d.data._collapsed ? '+' : '−');
      nodes.select('.osint-dot').attr('fill', d => _oColor(d)).attr('r', d => _oRadius(d.data.type));
      ...
      if (!_fitted) {
        _fitted = true;
        const padL = 160, padY = 30;
        const sx = (W - padL - 200) / treeW;
        const sy = (H - padY * 2)   / treeH;
        const sc = Math.min(sx, sy, 1.0);
        _svg.call(
          _zoom.transform,
          d3.zoomIdentity.translate(padL, (H - treeH * sc) / 2).scale(sc)
        );
      }
    }

    /* ── Toggle node expanded/collapsed ─────────────────────── */
    function _toggle(nodeId) {
      function find(n) {
        if (n.id === nodeId) {
      ...
      if (_root) { find(_root); _render(); }
    }

    /* ── Public controls (wired to toolbar buttons in card HTML) */
    function expand() {
      function exp(n) {
        if (n._hasChildren && n._collapsed) { n._collapsed = false; return; }
        (n.children || []).forEach(exp);
      }
      if (_root) { exp(_root); _render(); }
    }
    ...
    return { build, expand, collapse, reset };
  }

  /**
   * Data builder for scoped renderer.
   * Mirrors _buildOsintData() but takes a caller-provided nextId()
   * function so each scoped instance has its own ID namespace.
   */
  function _buildOsintDataWith(riskData, nextId) {
    const hosts = riskData?.hosts || [];
    const host  = hosts[0] || {};
    const ip    = host.ip || 'unknown';
    const rs    = host.risk_summary || {};
    ...
    const root = {
      id: `h-${nextId()}`, name: ip, type: 'host',
      riskLevel: rs.overall || 'low',
      meta: { scan: 'Host', ports: (host.ports || []).length, risk: (rs.overall || 'low').toUpperCase() },
      _collapsed: false, _hasChildren: (host.ports || []).length > 0, children: [],
    };

    for (const port of (host.ports || [])) {
      const pnum  = port.port, proto = port.protocol || 'tcp';
      const svc   = (port.service || 'unknown').toUpperCase();
      const rl    = port.risk?.level || 'low', score = port.risk?.score || 0;
      const va    = port.version_analysis || {};
      const prod  = port.product || '', ver = port.version || '';
      const cves  = port.cves || [], patch = port.patch || '';
      const chains = port.attack_chains || [];
      const portCh = [];
      ...
      if (prod || ver) {
        ...
        const vColor  = { latest: 'low', outdated: 'medium', unsupported: 'critical' }[vStatus] || 'low';
        portCh.push({
          id: `v-${nextId()}`, name: `${prod} ${ver}`.trim() || 'unknown version',
          type: 'version', riskLevel: vColor,
          meta: { status: vStatus, age: va.age_years ? va.age_years + 'y' : '?', eol: va.eol_date || '' },
          _collapsed: true, _hasChildren: false, children: [],
        });
      ...
      for (const cve of cves.slice(0, 6)) {
        const cid = cve.cve_id || '?', sev = cve.severity || 'unknown';
        const cvss = cve.cvss_score || 0, desc = cve.description || '', fix = cve.patch || '';
        portCh.push({
          id: `cve-${nextId()}`, name: cid, type: 'cve', riskLevel: sev,
          meta: { severity: sev.toUpperCase(), cvss, description: desc },
          _collapsed: true, _hasChildren: true,
          children: [
            { id: `cd-sev-${nextId()}`,  name: `${sev.toUpperCase()} · CVSS ${cvss}`, type: 'detail',     riskLevel: sev,  meta: { full: `Severity: ${sev.toUpperCase()}, CVSS: ${cvss}` }, _collapsed: true, _hasChildren: false, children: [] },
            { id: `cd-desc-${nextId()}`, name: desc.length > 46 ? desc.slice(0, 46) + '…' : desc || 'No description', type: 'detail', riskLevel: 'low', meta: { full: desc }, _collapsed: true, _hasChildren: false, children: [] },
            { id: `cd-fix-${nextId()}`,  name: fix ? (fix.length > 46 ? fix.slice(0, 46) + '…' : fix) : 'No patch info', type: 'detail_fix', riskLevel: 'low', meta: { full: fix || 'No patch guidance available.' }, _collapsed: true, _hasChildren: false, children: [] },
          ],
        });
      }
      ...
      for (const chain of chains.slice(0, 3)) {
        portCh.push({
          id: `atk-${nextId()}`, name: chain.name || 'Attack Chain', type: 'attack', riskLevel: 'critical',
          meta: { full: chain.narrative || '', score: chain.score || 0 },
          _collapsed: true, _hasChildren: false, children: [],
        });
      }
      ...
      if (patch) {
        portCh.push({
          id: `patch-${nextId()}`, name: patch.length > 46 ? patch.slice(0, 46) + '…' : patch,
          type: 'patch', riskLevel: 'low', meta: { full: patch },
          _collapsed: true, _hasChildren: false, children: [],
        });
      }

      root.children.push({
        id: `p-${nextId()}`, name: `${pnum}/${proto} · ${svc}`, type: 'port', riskLevel: rl,
        meta: { port: pnum, service: svc, risk: rl.toUpperCase(), score, protocol: proto },
        _collapsed: true, _hasChildren: portCh.length > 0, children: portCh,
      });
    }
    return root;
  }
  ...
  return {
    ...
    renderTrendCharts,
    // OSINT tree (dashboard — global state)
    isBuilt,
    markStale,
    clearOsint,
    buildOsintTree,
    ...
    osintCollapse,
    osintReset,
    // Scoped OSINT tree (inline /graph chat card)
    buildOsintTreeInContainer,
  };
