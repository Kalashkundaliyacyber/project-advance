...
const SessionManager = (() => {
  const LS_KEY          = 'ThreatWeave_sessions_v4';
  const LS_ACTIVE       = 'ThreatWeave_active_v4';
  const LS_SCROLL       = 'ThreatWeave_scroll_v4';
  const LS_STARTUP_TOK  = 'ThreatWeave_startup_token_v4';
  const MAX_MSGS        = 500;
  ...
  async function checkStartupToken() {
    try {
      ...
      if (serverToken && serverToken !== cachedToken) {
        console.info('[ThreatWeave] New run.sh detected — fresh chat. Token:', serverToken);
      ...
      console.warn('[ThreatWeave] startup-token fetch failed, defaulting to fresh:', e);
      localStorage.removeItem(LS_ACTIVE);
      return 'fresh';
    }
  }
  ...
  /**
   * clearAllSessions — wipe EVERY session from memory and localStorage.
   * Called on fresh server start (run.sh re-executed) so that ALL browsers
   * — Chrome, Firefox, mobile — get a completely clean slate.
   * Without this, named sessions with messages survived purgeBlankSessions()
   * and kept appearing in the project history sidebar after every restart.
   */
  function clearAllSessions() {
    const count = Object.keys(_sessions).length;
    _sessions  = {};
    _activeId  = null;
    _scrollPos = {};
    try { localStorage.removeItem(LS_KEY);    } catch (_) {}
    try { localStorage.removeItem(LS_ACTIVE); } catch (_) {}
    try { localStorage.removeItem(LS_SCROLL); } catch (_) {}
    console.info('[SessionManager] clearAllSessions: wiped ' + count + ' session(s) from localStorage on fresh server start.');
  }
