 ...
 */
  ...
  /** Restore a completed live vuln table from session history */
  function _buildLiveVulnTableEl(data, container) {
    const d = document.createElement('div');
    d.className = 'msg msg-ai live-vuln-wrap live-vuln-restored';
    d.innerHTML = `
      <div class="lv-header">
        <div class="lv-title-row">
          <span class="lv-scan-badge complete">✔ COMPLETE</span>
          <span class="lv-target"><code>${data.ip || '?'}</code></span>
          <span class="lv-scan-label">nmap -sV --script vuln</span>
        </div>
        <div class="lv-counters">
          <span class="lv-ctr lv-ctr-total">(results in table below)</span>
        </div>
      </div>`;
    container.appendChild(d);
  }
