...
def _nvd_lookup(product: str, version: str) -> list:
    """Query NVD for a product+version. Checks intelligent JSON cache first."""
    # ── 1. Try intelligent JSON cache (zero tokens, zero API calls) ───────────
    try:
        from app.cve.cve_cache_engine import cve_cache
        cached = cve_cache.lookup_by_product(product, version)
        if cached:
            logger.debug("[CVE] Cache hit for '%s %s' → %d CVEs", product, version, len(cached))
            return cached
    except Exception as ce:
        logger.debug("[CVE] Cache lookup failed: %s", ce)

    # ── 2. Fall through to direct NVD HTTP call (legacy path) ─────────────────
    cache_key = f"{product}_{version}"
    cached_file = _load_cache(cache_key)
    ...
    try:
        ...
        req.add_header("User-Agent", "ThreatWeave/2.0")
        ...
        # ── Also save each CVE individually into the intelligent JSON cache ──
        try:
            from app.cve.cve_cache_engine import cve_cache
            for r in results[:10]:
                r.setdefault("product_hint", product)
                cve_cache._store(r)          # noqa: private but intentional
        except Exception:
            pass
...
def map_cves(versioned: dict) -> dict:
    ...
    for host in result.get("hosts", []):
        for port in host.get("ports", []):
            port["cves"] = _find_cves(port, use_nvd=False)
    return result
...
def _find_cves(port: dict, use_nvd: bool = False) -> list:
    ...
    # NVD live lookup disabled — use nvd_client.py for async enrichment instead
