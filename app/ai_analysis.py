...
logger = logging.getLogger("ThreatWeave.ai_analysis")
...
def analyze_scan(data: dict) -> dict:
    ...
    try:
        ...
        text, provider = ai_router.generate(
            prompt, system=system, expect_json=True, max_tokens=1024  # Reduced from 4096 to lower RAM
        )
...
def explain_cve(cve_id: str, service: str, version: str = "unknown",
                cvss: float = 0.0, severity: str = "unknown",
                description: str = "", cwe: str = "",
                epss: float = 0.0, is_kev: bool = False) -> dict:
    """
    Get a structured 3-tier CVE explanation.
    Feature 1: Structured CVE Summarization — executive, technical, remediation summaries.
    Falls back to AI explain_cve prompt if summarizer unavailable.
    """
    # Fast path: use structured summarizer (Feature 1)
    try:
        from app.intelligence.cve_summarizer import summarize_cve
        summary = summarize_cve(
            cve_id=cve_id, service=service, version=version,
            cvss=cvss, severity=severity, description=description,
            cwe=cwe, epss=epss, is_kev=is_kev,
        )
        # Augment with legacy fields for backwards compatibility
        summary.setdefault("cve_id", cve_id)
        summary.setdefault("title", f"{cve_id} — {service}")
        summary.setdefault("mitigation", summary.get("remediation", f"Apply latest patches for {service}."))
        return summary
    except Exception:
        pass

    # Fallback: legacy single-prompt explain
...
{{
  ...
  "executive_summary": "<1-2 sentence management summary>",
  "technical_summary": "<2-3 sentence technical detail>",
  "description": "<2-3 sentence defensive-focused explanation>",
  "affected_versions": "<version range>",
  "fixed_in": "<fixed version>",
  "remediation": "<step-by-step patch/workaround>",
  ...
  "urgency": "<immediate|urgent|scheduled|low>"
    ...
    try:
        text, provider = ai_router.generate(
            prompt, system=system, expect_json=True, max_tokens=1024, task_type="security"
        )
        result = safe_parse_json(text)
        if isinstance(result, dict) and result:
            result["engine"] = provider
            result.setdefault("executive_summary", result.get("description", ""))
            result.setdefault("technical_summary", result.get("description", ""))
            result.setdefault("remediation", result.get("mitigation", f"Apply latest patches for {service}."))
            return result
        ...
        return {
            ...
            "executive_summary": f"A vulnerability ({cve_id}) was detected in {service} {version}. Apply patches immediately.",
            "technical_summary": f"Unable to fetch full technical details. Consult https://nvd.nist.gov/vuln/detail/{cve_id}",
            "description":       f"Unable to fetch CVE details. Consult https://nvd.nist.gov/vuln/detail/{cve_id}",
            "remediation":       f"Apply the latest security update for {service}.",
...
def _rule_based_analyze(data: dict) -> dict:
    ...
    return {
        ...
        "notes": [
            ...
            "Run setup_env.sh to install Ollama and pull qwen2.5-coder:7b + llama3.2:3b",
        ],
        "engine":       "rule-based",
