...
logger = logging.getLogger("threatweave.risk_engine")
        ...
        for k in defaults:
            if k in rw:
                try:
                    ...
                    logger.warning(
                        "_load_weights: invalid value for risk_weights.%s=%r in settings.yaml: %s — using default %.2f",
                        k, rw[k], exc, defaults[k]
                    )
    except FileNotFoundError:
        pass  # settings.yaml not present — use env/defaults (expected in most deployments)
    except Exception as exc:
        logger.warning("_load_weights: could not read settings.yaml: %s — using env/defaults", exc)
