"""
ThreatWeave — Remediation Templates (Rule-Based Fallback)
...
def build_patch_response(service: str, port: int, version: str,
    ...
    return {
        ...
        "mitigation":        "\n".join(
            f"• {fix}" for fix in tmpl.get("recommended_fixes", [
                f"Restrict port {port} via firewall if patching is delayed."
            ])
        ),
