...
SCAN_TEMPLATES = {
    ...
    "vuln_scan": {
        ...
        "args":           ["-p-", "-v", "-sC", "--script", "vuln", "-sV", "-O",
                           "--min-rate", "1000", "--max-rtt-timeout", "100ms",
                           "--max-retries", "5", "-oX", "-"],
        "requires_root":  True,
        "risk_level":     "aggressive",
        "estimated_time": "~5-10m",
        "tags":           ["vuln", "cve", "nse", "p-", "os", "fast"],
        "recommended":    True,
        "advanced":       False,
    },
    ...
    # Phase 15: Network Inventory scan
    "network_inventory": {
        "label":       "Network Inventory",
        "description": "Discover all hosts and services on a subnet. Best for asset mapping.",
        "nmap_args":   ["-sn", "-T4", "--open"],
        "intensity":   "light",
        "estimated_seconds": 30,
        "icon":        "🗺️",
        "tags":        ["discovery", "inventory", "subnet"],
    },
    # Phase 15: Comprehensive (combined) scan
    "comprehensive": {
        "label":       "Comprehensive Scan",
        "description": "Full port + version + OS + scripts. Most thorough, slowest.",
        "nmap_args":   ["-sV", "-O", "-sC", "-p-", "-T3", "--open"],
        "intensity":   "deep",
        "estimated_seconds": 600,
        "icon":        "🔬",
        "tags":        ["full", "comprehensive", "deep"],
    },
    # Phase 15: Risk Assessment scan
    "risk_assessment": {
        "label":       "Risk Assessment",
        "description": "Version detection + CVE lookup + risk scoring. Best for compliance reports.",
        "nmap_args":   ["-sV", "--version-intensity", "5", "-T4", "--open", "-p-"],
        "intensity":   "medium",
        "estimated_seconds": 180,
        "icon":        "⚠️",
        "tags":        ["risk", "cve", "compliance"],
    },
        "port_range": {
...
def get_scan_command(scan_type: str, target: str) -> list:
    ...
    FIX: Injects -Pn for all non-discovery scan types so nmap treats the host
    as online and skips host-discovery probes.  Without -Pn, hosts that block
    ICMP/TCP ping cause every scan to return 0 hosts in ~13 s because nmap
    marks them as "down" and never reaches port scanning.

    Discovery-only types (ping_sweep, host_discovery, arp_discovery) keep their
    native host-discovery behaviour so they can map live hosts on a subnet.
    """
    ...
    args = list(SCAN_TEMPLATES[scan_type]["args"])

    # These scans exist solely to discover which hosts are up — keep host discovery.
    _DISCOVERY_ONLY = {"ping_sweep", "host_discovery", "arp_discovery"}

    # Inject -Pn for all port/vuln/enum/advanced scans unless already present.
    if scan_type not in _DISCOVERY_ONLY and "-Pn" not in args:
        return ["nmap", "-Pn"] + args + [target]
...
# Phase 15: backward-compat alias
SCAN_PROFILES = SCAN_TEMPLATES
