...
def broadcast_port_event(port_data: dict):
    """Broadcast a live port_found SSE event to all connected frontend listeners."""
    try:
        from app.api.scan_control import scan_state
        scan_state.broadcast_port_event(port_data)
    except Exception:
        pass
...
def analyze_port_vuln_status(port_xml_fragment: str) -> dict:
    ...
    run vuln status analysis, and return the enriched port dict.
    Used during live streaming so each port fires an SSE event as soon as nmap
    closes its </port> tag.
    """
    try:
        elem = _ET.fromstring(port_xml_fragment)
    except _ET.ParseError:
        return {}

    try:
        from app.parser.nmap_parser import _parse_port
        port = _parse_port(elem)
        return port or {}
    except Exception:
        return {}
...
def execute_scan(cmd: list, target: str, scan_type: str):
    ...
    LIVE STREAMING: While nmap runs, stdout (XML) is read line-by-line.
    When a complete <port>…</port> block is detected, it is immediately
    parsed and broadcast as a `port_found` SSE event to the frontend.
    """
    ...
    try:
        ...
        # ── Live per-port XML streaming ────────────────────────────────────
        # Read stdout line-by-line. Buffer <port> blocks and fire SSE events
        # the moment </port> is detected — before nmap finishes.
        stdout_lines: list = []
        stderr_lines: list = []
        _port_buf: list    = []
        _in_port: bool     = False
        ...
        _stdout_thread = _threading.Thread(target=_read_stdout, daemon=True)
...
def _simulated_scan(target: str, scan_type: str):
    ...
    Also fires live port_found SSE events so the frontend live table works
    ...
    # Fire live port_found events from simulation so the live table works
    _fire_simulated_port_events(target, xml)
...
def _sim_vuln(target):
    """Simulated vuln_scan output — includes realistic NSE script results so
    the live table shows CONFIRMED / NOT_VULNERABLE / UNCONFIRMED statuses."""
    return f"""<?xml version="1.0"?>
<nmaprun scanner="nmap" args="nmap -sV --script vuln {target}" version="7.95" xmloutputversion="1.05">
<host starttime="1720000000" endtime="1720000120">
...
<ports>
  ...
  <port protocol="tcp" portid="443">
    ...
    <script id="http-vuln-cve2014-3704" output="&#xa;  NOT VULNERABLE&#xa;  Target is not running Drupal or version is patched."/>
  </port>
  ...
  <port protocol="tcp" portid="8080">
    <state state="open" reason="syn-ack"/>
    <service name="http-proxy" product="Apache Tomcat" version="6.0.35" conf="10" method="probed"/>
    <script id="http-vuln-cve2013-6357" output="&#xa;  Script ran, no definitive state found for this target."/>
  </port>
</ports>
